...
def _sel_all(soup_or_el, sel: Selector):
    if sel.type == "css":
...
def extract_items(html: str, selectors: FieldMap, container_css: Optional[str] = None, item_css: Optional[str] = None) -> List[Dict]:
    ...
    if item_css:
        ...
        for it in items:
            ...
            row["url"] = _get_text_or_attr((_sel_all(it, selectors.url) or [None])[0], selectors.url.attr)
            if selectors.date:
    ...
    urls = _sel_all(scope, selectors.url)
...
def selector_diagnostics(html: str, selectors: FieldMap, container_css: Optional[str], item_css: Optional[str]) -> Dict[str, int]:
    ...
    if selectors.date: counts["date_matches"] = len(_sel_all(scope, selectors.date))
    if selectors.summary: counts["summary_matches"] = len(_sel_all(scope, selectors.summary))
    if selectors.section: counts["section_matches"] = len(_sel_all(scope, selectors.section))
    return counts
